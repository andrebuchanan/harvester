...
var
  ...
  convert = require("./dataConverters/converter"),  store   = require("./awsStore");
var
  log     = util.log;
...
http.get("/otp/:fleet/:date?", function(req, res)
{
  ...
  store.getOTP(getRouteParams(req.params), function(error, data)
  {
    log("response from STORE received: " + error + " : " + data);
    res.json(200, convert.convertOTP(data, req.params.date));
  });
});
