...
module.exports.cache = function(cache, category, keyFields)
{
  // Don't do anything if cache is not ready.
  if (!_cacheReady) return false;

  // If not cache, return through.
  if (!cache) return es.mapSync(function(data)
  {
    return data;
  });
  ...
  return es.map(function(data, doneCb)
  {
    ...
    {
      // This is a passthrough; we want to ensure data goes out of here
      // no matter what.
      doneCb(null, data);
    });
