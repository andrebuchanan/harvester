...
//
// FIDS by fleet.
http.get("/fids/fleet/:fleet", function(req, res)
{
...
http.param("date", function(req, res, next, date)
{
  ...
  {
    // XXX Should convert date on ingest, not here.
    req.dateConversion = moment(date).format("MM/DD/YYYY");
    log(req.dateConversion);
    next();
  }
