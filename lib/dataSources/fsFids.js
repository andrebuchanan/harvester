...
exports.dataType = "fids";
exports.keyFields = ["fcode", "trans"];

// Open the fids url file and parse contents to json structure. Expect
// file contents to be array of objects.
var urls = require("./fsFids.json");
// Light error checking on urls structure.
urls.forEach(function(url)
{
  if (!(url.url && url.portCode)) throw "FIDS url definition not complete.";
  url.type = exports.dataType;
  urlCtrl.addUrl(url);
});
...
if (urls.length === 0)
{
  throw "No urls defined for FIDS data source.";
}
...
var reqOptions = {
  ...
  url: null, // Update in the requester
  qs: {
    "appId": appId,
    "appKey": appKey,
...
function collect(harvesterCb)
{
  ...
  return requester.collect(
    ...
    function(url, error, body)
    {
      ...
      if (body && body.fidsData)
      {
        log("Collected [" + body.fidsData.length + "] FIDS entries.");
        body.portCode = url.portCode;
        body.transition = url.transition;
      }
...
exports.auger = function(input)
{
  ...
  return mapProc.processData(input.fidsData, dMap, null, recordCb);
}
