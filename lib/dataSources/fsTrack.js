...
function thresh(options)
{
  ...
  var interval = options.threshInt || (5 * 60000);
  var fsKey = options.fsKey;

  // If there is no key, cease and desist.
  if (!fsKey)
  {
    console.log("Threshing cannot begin without an API key.");
    return false;
  }
  ...
  request(reqOptions, function(error, res, body)
  {
    ...
    data.flightPositions.forEach(function(item)
    {
